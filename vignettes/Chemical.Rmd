...
In this vignette, [Computational Toxicology and Exposure Data (CTED) API Chemical endpoints](https://api-ccte.epa.gov/docs/chemical.html) will be explored. 
...
With cheminformatics as the backbone for research efforts, sources of available data through the CTED API Chemical endpoints include:
...
Several ccdR functions are used to access the CTED API Chemical data.
...
### Structure from IUPAC Systematic 

#### Get Smiles

`get_smiles()` retrieves a SMILES string for the input chemical name.

```{r smiles from chemical name, message = FALSE, eval = FALSE}
res_string <- get_smiles(name = 'Bisphenol A')
```

#### Get inchikey

`get_inchikey()` retrieves the InChIKey string for the input chemical name.

```{r inchikey from chemical name, message = FALSE, eval = FALSE}
res_string <- get_inchikey(name = 'Bisphenol A')
```

#### Get inchi

`get_inchi()` retrieves the InChI string for the input chemical name.

```{r inchi string from chemical name, message = FALSE, eval = FALSE}
res_string <- get_inchi(name = 'Bisphenol A')
```
...
##### Mass and error offset

```{r ccdR msready mass and error offset, message=FALSE, eval=FALSE}
res_list <- get_msready_by_mass_with_error_batch(masses = c(226, 228), error = 10)
```
