...
```{r}
ccl4_hazard[toxvalType %in% c('NOEL', 'NOEC', 'LOEL', 'LOEC'), .(.N, by_media = length(unique(toxvalUnits))), by = .(media, toxvalType)][order(by_media, -N),]
natadb_hazard[toxvalType %in% c('NOEL', 'NOEC', 'LOEL', 'LOEC'), .(.N, by_media = length(unique(toxvalUnits))), by = .(media, toxvalType)][order(by_media, -N),]
```
...
```{r}
ccl4_hazard[media %in% 'soil' & toxvalType %in% c('LOEL', 'NOEL'), .(dtxsid = unique(dtxsid), toxvalNumeric = which.min(toxvalNumeric)), by = .(toxvalType, toxvalUnits)]
natadb_hazard[media %in% 'soil' & toxvalType %in% c('LOEL', 'NOEL'), .(dtxsid = unique(dtxsid), toxvalNumeric = which.min(toxvalNumeric)), by = .(toxvalType, toxvalUnits)]
```
