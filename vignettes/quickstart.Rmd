...
Note that input strings may include URLencoded chemical names, DTXSIDs, DTXCIDs, InChIKeys, and CASRNs. For `chemical_starts_with()`, the InChIKey string needs to include the first 13 characters.
...
methane_df <- get_chemical_details_batch(DTXSID = methane_mass[1:10],
                                              API_key = my_key)
...
```{r, mol-formula-batch}
methane_formula_list <- methane_df$molFormula
...
```{r, cancer}
phenolphthalein_cancer <- get_cancer_hazard(DTXSID = 'DTXSID0021125',
                                API_key = my_key)
phenolphthalein_cancer
```

### Genetox resource

Finally we explore the genetox resource. This allows us to grab either summary or detail level genetox data. We first demonstrate the summary endpoint.
