...
# Build pause to prevent overloading the APIs
Sys.sleep(5)
```
...
```{r}
methane_df_list <- lapply(methane_mass[1:10], function(t) {
  get_chemical_details(DTXSID = t, API_key = my_key)
})
...
```{r}
methane_formula_list <- sapply(methane_mass[1:10], function(t) {
  get_chemical_details(DTXSID = t, API_key = my_key)$molFormula
})
