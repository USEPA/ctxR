...
#### Get human hazard by dtxsid

`get_human_hazard_by_dtxsid()` retrieves only the human hazard data.

```{r ccdR human hazard, message=FALSE, eval=FALSE}
res_dt <- get_human_hazard_by_dtxsid(DTXSID = 'DTXSID7020182')
```

#### Get ecotox hazard by dtxsid

`get_ecotox_hazard_by_dtxsid()` retrieves only the ecotox hazard data.
...
`get_genetox_summary()` retrieves summary level data for genetox data associated to a chemical.
