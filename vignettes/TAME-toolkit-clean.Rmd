...
Accessing chemical data and wrangling it is a vital step in many types of workflows related to chemical, biological, and environmental modeling. While there are many resources available from which one can pull data, the [CompTox Chemicals Dashboard](https://comptox.epa.gov/dashboard/) built and maintained by the United States Environmental Protection Agency is particularly well-designed and maintained. Originally introduced in [The CompTox Chemistry Dashboard: a community data resource for environmental chemistry](https://jcheminf.biomedcentral.com/articles/10.1186/s13321-017-0247-6), the CCD contains information on over 1.2 million chemicals as of December, 2023. 
...
The CCD can be searched either one chemical at a time, or in batch mode.
...
![Fig 1: Bisphenol A landing page](./Pictures/CCD BPA page.png){width=120%}
...
![Fig 2: CCD Batch Search](./Pictures/CCD Batch Search.png){width=120%}
...
![Fig 3: CCD Batch Search Customize Export Results](./Pictures/CCD Batch Search customize export results.png){width=120%}
...
Recently, the [Center for Computational Toxicology and Exposure](https://www.epa.gov/aboutepa/about-center-computational-toxicology-and-exposure-ccte) (CCTE) developed a set of Application Programming Interfaces (APIs) that allows programmatic access to the CCD, bypassing the manual steps of the web-based batch search workflow. The [CCTE APIs](https://api-ccte.epa.gov/docs/) are publicly available at no cost to the user. 

In order to use the CCTE APIs, you must have an API key. The API key uniquely identifies you to the CCD servers and verifies that you have permission to access the database. Getting an API key is free, but requires contacting the API support team at [ccte_api@epa.gov](mailto:ccte_api@epa.gov).
...
![Fig 4: CCTE API Chemical Endpoints](./Pictures/CCTE Chemical API endpoints.png){width=120%}

The APIs can be explored through the pictured web interface at https://api-ccte.epa.gov/docs/chemical.html . 
`Authentication` is the first tab on the left. Authentication is required to use the APIs. To authenticate yourself, input your unique API key as demonstrated in Figure 5.

![Fig 5: CCTE API Key Authentication](./Pictures/CCTE API Key authentication.png){width=120%}

Below the `Authentication` tab, there are several different tabs, one for each endpoint in the `Chemical` domain. The endpoints are organized by the type of information provided. For instance, the `Chemical Details Resource` endpoint provides basic chemical information; the `Chemical Property Resource` endpoint provides more comprehensive physico-chemical property information; the `Chemical Fate Resource` endpoint provides chemical fate and transport information; and so on. 

The colored boxes next to each endpoint indicate the type of the associated `http` method: either a GET request ("GET", blue) or a a POST request ("POS", green). If you aren't familiar with `http` methods, GET is used to request data from a specific web resource; POST is used to send data to a server to create or update a web resource. POST requests are used to perform multiple (batch) searches in a single API call; GET requests are used for non-batch searches. (You do not need to understand the details of POST and GET requests in order to use the API.)

Clicking on the `Get data by DTXSID` endpoint, we observe that there are certain required pieces of information to construct the request and optional parameters to focus the request.

![Fig 6: CCTE Get Details by DTXSID](./Pictures/Get Data by DTXSID TOP.png){width=120%}
...
![Fig 7: CCTE Get Details by DTXSID data types](./Pictures/Get Data by DTXSID BOTTOM.png){width=120%}
...
![Fig 8: Bisphenol A output](./Pictures/BPA chemicaldetailstandard.png){width=120%}
...
Now we store the API key required to access the APIs. To do this for the current session, run the first command. If you want to store your key across multiple sessions, run the second command.
...
ccte_key()
```
Now, we demonstrate how to retrieve the information for BPA given by the `Chemical Detail Resource` endpoint under the "chemicaldetailstandard" `projection`. Note, this `projection` value is the default value for the function `get_chemical_details()`.
...
![Fig 9: CompTox Chemicals Dashboard Genotoxicity](./Pictures/BPA Hazard data - Genotoxicity.png){width=120%}
...
![Fig 10: CompTox Chemicals Dashboard BPA Genotoxicity](./Pictures/CCD BPA genotoxicity.png){width=120%}
...
![Fig 11: CCTE APIs Hazard endpoint gentoxocity resources](./Pictures/Hazard - genotoxicity resource.png){width=120%}
...
The information returned is of the first variety highlighted in the Figure 9, that is, summary data on the available genotoxicity data for each chemical.
...
Determine the number of assays per unique `assayCategory`.
